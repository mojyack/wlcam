...
auto main(const int argc, const char* const argv[]) -> int {
    ...
    v4l2::set_format_subdev(imgu_0.imgu, imgu_0.imgu_input_pad_index, MEDIA_BUS_FMT_FIXED, args.width, args.height); // GDC
    ...
    v4l2::set_selection_subdev(imgu_0.imgu, imgu_0.imgu_input_pad_index, V4L2_SEL_TGT_CROP, 0, 0, args.sensor_width, args.sensor_height);    // IF
    v4l2::set_selection_subdev(imgu_0.imgu, imgu_0.imgu_input_pad_index, V4L2_SEL_TGT_COMPOSE, 0, 0, args.sensor_width, args.sensor_height); // BDS
    ...
    auto camera_thread        = std::thread([&]() {
        ...
        auto       ubuf           = std::vector<std::byte>(height / 4 * stride);
        auto       vbuf           = std::vector<std::byte>(height / 4 * stride);
