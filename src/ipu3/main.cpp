...
auto main(const int argc, const char* const argv[]) -> int {
    ...
    const auto viewfinder      = algo::calculate_best_viewfinder(output, {1920, 1280}); // TODO
    ...
    const auto imgu_vf_fmt = v4l2::set_format_mp(imgu_0.viewfinder, V4L2_BUF_TYPE_VIDEO_CAPTURE_MPLANE, V4L2_PIX_FMT_NV12, 2, output.width, output.height, nullptr);
    ...
    v4l2::request_buffers(imgu_0.viewfinder, V4L2_BUF_TYPE_VIDEO_CAPTURE_MPLANE, V4L2_MEMORY_MMAP, 0);
    ...
    for(auto i = 0u; i < num_buffers; i += 1) {
        vf_mmap_ptrs[i] = mmap(NULL, imgu_vf_buffers[i].length,
                               PROT_READ | PROT_WRITE,
                               MAP_SHARED, imgu_vf_buffers[i].fd, 0);
    ...
    auto camera_thread        = std::thread([&]() {
        ...
        while(!finish_camera_thread) {
            ...
            v4l2::queue_buffer_mp(imgu_0.output, V4L2_BUF_TYPE_VIDEO_CAPTURE_MPLANE, V4L2_MEMORY_DMABUF, i, &imgu_output_buffers[i], 1);
            v4l2::queue_buffer_mp(imgu_0.viewfinder, V4L2_BUF_TYPE_VIDEO_CAPTURE_MPLANE, V4L2_MEMORY_DMABUF, i, &imgu_vf_buffers[i], 1);
            ...
            v4l2::queue_buffer_mp(imgu_0.input, V4L2_BUF_TYPE_VIDEO_OUTPUT_MPLANE, V4L2_MEMORY_DMABUF, i, &cio2_output_buffers[i], 1);

            v4l2::dequeue_buffer_mp(imgu_0.output, V4L2_BUF_TYPE_VIDEO_CAPTURE_MPLANE, V4L2_MEMORY_DMABUF);
            v4l2::dequeue_buffer_mp(imgu_0.viewfinder, V4L2_BUF_TYPE_VIDEO_CAPTURE_MPLANE, V4L2_MEMORY_DMABUF);
