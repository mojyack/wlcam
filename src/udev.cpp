...
auto enumerate() -> std::optional<std::unordered_map<dev_t, std::string>> {
    ...
    ensure(udev_enum.get() != NULL);

    ensure(udev_enumerate_add_match_subsystem(udev_enum.get(), "media") >= 0);
    ensure(udev_enumerate_add_match_subsystem(udev_enum.get(), "video4linux") >= 0);
    ensure(udev_enumerate_add_match_is_initialized(udev_enum.get()) >= 0);
    ensure(udev_enumerate_scan_devices(udev_enum.get()) >= 0);

    unwrap_mut(ents, udev_enumerate_get_list_entry(udev_enum.get()));
    ...
    for(auto ent = &ents; ent; ent = udev_list_entry_get_next(ent)) {
        ...
        unwrap_mut(devnode, udev_device_get_devnode(dev.get()));
